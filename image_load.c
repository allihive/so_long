...
t_img *load_taylor_texture(mlx_t *mlx, t_img *img)
{
    ...
    if (!img->taylor)
        error_msg("Taylor can't come to the game right now, txtre->img not loaded");
...
t_img *load_phone_texture(mlx_t *mlx, t_img *img)
{
    ...
    if (!phone)
        error_msg("brand new phone dropped on the cobble stone, png hasn't loaded");
    img->phonebooth = mlx_texture_to_image(mlx, phone);
    if (!img->phonebooth)
        error_msg("brand new phone dropped on the cobble stone, txtre->img not loaded");
...
t_img *load_vinyl_texture(mlx_t *mlx, t_img *img)//what happened here?
{
    ...
    if (!vinyl)
        error_msg("burned my cd, png hasn't loaded");
    img->vinyl = mlx_texture_to_image(mlx, vinyl);
    if (!img->vinyl)
        error_msg("burned my cd, txtre->img not loaded");
...
t_img *load_willow_texture(mlx_t *mlx, t_img *img)
{
    ...
    if (!willow)
        error_msg("life was a willow and it bent wrong to the wind, png hasn't loaded");
    ...
    while (y < game->y)
    {
        ...
        while (x < game->x)
        {
            if (mlx_image_to_window(game->mlx, game->img->cobblestones, x * PIXELS, y * PIXELS) < 0)
                error_msg("just another picture that burned");
            x++;
        }
        y++;
    }
